. .\BuildFunctions.ps1
$projectName = "CleanApi_dotnet6"
$base_dir = resolve-path .\
$source_dir = "$base_dir\src"
$test_dir = "$base_dir\tests"
$unitTestProjectPath = "$test_dir\UnitTests"
$integrationTestProjectPath = "$test_dir\IntegrationTests"
$acceptanceTestProjectPath = "$test_dir\AcceptanceTests"
$apiProjectPath = "$source_dir\ApiServer"
$dbProjectPath = "$source_dir\ApiServer"
...
Function PackageUI {    
    exec{
        & dotnet publish $apiProjectPath -nologo --no-restore --no-build -v $verbosity --configuration $projectConfig
    }
	exec{
		& dotnet-octo pack --id "$projectName.UI" --version $version --basePath $apiProjectPath\bin\$projectConfig\$framework\publish --outFolder $build_dir --overwrite
	}
}

Function PackageDatabase {    
    exec{
		& dotnet-octo pack --id "$projectName.Database" --version $version --basePath $dbProjectPath --outFolder $build_dir --overwrite
	}
}
