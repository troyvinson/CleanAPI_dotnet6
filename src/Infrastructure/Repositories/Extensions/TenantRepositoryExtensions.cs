...
public static class TenantRepositoryExtensions
{
    ...
    public static IQueryable<Tenant> Search(this IQueryable<Tenant> tenants, string? searchTerm)
    {
        ...
        return tenants.Where(e => e.Name!.ToLower().Contains(lowerCaseTerm));
    }

    /// <summary>
    /// Sorts the tenants by the URL "orderBy" query parameter instruction or by Id if invalid or empty.
    /// </summary>
    /// <param name="tenants"></param>
